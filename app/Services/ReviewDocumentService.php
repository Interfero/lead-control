...
class ReviewDocumentService
{
     ...
     * @throws ValidationException
     */
    public function attachToReview(Review $review, UploadedFile $file, int $uploadedByUserId): Document
    {
        $this->assertMimeAllowed($file);

        $safeName = $this->safeOriginalFileName($file);
        $fileName = time().'_'.$safeName;
        $folder = "documents/reviews/{$review->id}";
        ...
        try {
            $document = new Document([
                'documentable_type' => Review::class,
                'documentable_id' => $review->id,
