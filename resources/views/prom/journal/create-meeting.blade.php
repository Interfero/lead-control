    ...
    <div class="card">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
            <h1 style="font-size: 1.25rem; font-weight: 600; margin: 0;">
                {!! icon('plus') !!} Новая встреча
            </h1>
        </div>
        
        <form method="POST" action="{{ route('prom.meetings.store') }}">
            @csrf
            ...
            <div class="form-row">
                <div class="form-group" style="flex: 1;">
                    ...
                    <input type="date" name="meeting_date" class="form-input" 
                           value="{{ old('meeting_date', now()->format('Y-m-d')) }}" required>
                    @error('meeting_date')
                ...
                <div class="form-group" style="flex: 1;">
                    ...
                    <input type="time" name="meeting_time" class="form-input" 
                           value="{{ old('meeting_time', now()->format('H:i')) }}" required>
                    @error('meeting_time')
                        <span class="form-error">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="form-row">
                ...
                <div class="form-group" style="flex: 1;">
                    ...
                    <input type="number" name="person_age" class="form-input" 
                           value="{{ old('person_age') }}" min="14" max="99">
                    @error('person_age')
                        <span class="form-error">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="form-group">
                ...
                <textarea name="person_address" class="form-input" rows="2">{{ old('person_address') }}</textarea>
            ...
            <div class="form-row">
                ...
                <div class="form-group" style="flex: 1;">
                    ...
                    <select name="person_source" class="form-input">
                        ...
                        <option value="head_hunter" {{ old('person_source') === 'head_hunter' ? 'selected' : '' }}>Head Hunter</option>
                        <option value="olx" {{ old('person_source') === 'olx' ? 'selected' : '' }}>OLX</option>
                        <option value="recommendation" {{ old('person_source') === 'recommendation' ? 'selected' : '' }}>Рекомендация</option>
                    </select>
                    @error('person_source')
                        <span class="form-error">{{ $message }}</span>
                    @enderror
                ...
                <div class="form-group" style="flex: 1;">
                    ...
                    <select name="maket_id" class="form-input">
                        ...
                        @foreach($makets as $maket)
                            <option value="{{ $maket->maket_id }}" {{ old('maket_id') == $maket->maket_id ? 'selected' : '' }}>
                                {{ $maket->maket_name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="form-row">
                <div class="form-group" style="flex: 1;">
                    ...
                    <input type="number" name="leaflets_issued" class="form-input" 
                           value="{{ old('leaflets_issued', 0) }}" min="0">
                ...
                <div class="form-group" style="flex: 1;">
                    ...
                    <select name="meeting_status" class="form-input" required>
                        @foreach($statuses as $code => $label)
                            <option value="{{ $code }}" {{ old('meeting_status', 'in_progress') === $code ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="form-group">
                ...
                <textarea name="meeting_comment" class="form-input" rows="2">{{ old('meeting_comment') }}</textarea>
