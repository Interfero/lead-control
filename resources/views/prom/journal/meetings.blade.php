...
<div style="max-width: 1600px; margin: 0 auto;">
    ...
    <div class="card" style="margin-bottom: 1rem; padding: 0.75rem;">
        <form method="GET" action="{{ route('prom.journal.meetings') }}" id="filtersForm">
            <div style="display: flex; gap: 0.75rem; flex-wrap: wrap; align-items: center;">
                ...
                <a href="{{ route('prom.journal.meetings', ['quick' => 'today']) }}" class="btn btn-filter">Сегодня</a>
                <a href="{{ route('prom.journal.meetings', ['quick' => 'tomorrow']) }}" class="btn btn-filter">Завтра</a>
                <a href="{{ route('prom.journal.meetings') }}" class="btn btn-filter">{!! icon('refresh') !!} Сбросить</a>
            </div>
        </form>
    </div>
    ...
    <div class="card" style="padding: 0;">
        <table class="table">
            ...
            <tbody>
                    ...
                    <tr>
                        <td>
                            @php
                                ...
                                $weekday = $weekdays[$meeting->meeting_datetime->dayOfWeek];
                            @endphp
                            <div style="display: flex; flex-direction: column; line-height: 1.3;">
                                <span style="font-weight: 600; font-size: 1rem;">{{ $meeting->meeting_datetime->format('H:i') }}</span>
                                <span style="font-size: 0.8125rem; color: #374151;">{{ $weekday }}</span>
                                <span style="font-size: 0.75rem; color: #374151;">{{ $meeting->meeting_datetime->format('d.m.Y') }}</span>
                            </div>
                        </td>
                        <td>{{ $meeting->city?->city_name ?? '—' }}</td>
                        <td>
                            @if($meeting->person_phone)
                                <x-phone-masked :number="$meeting->person_phone" :show-adds="false" />
                            @else
                                —
                            @endif
                        </td>
                        <td>{{ $meeting->person_telegram ?? '—' }}</td>
                        <td>{{ $meeting->person_age ?? '—' }}</td>
                        <td><strong>{{ $meeting->person_name }}</strong></td>
                        <td style="max-width: 200px;">
                            <span style="display: -webkit-box; -webkit-line-clamp: 1; -webkit-box-orient: vertical; overflow: hidden;" 
                                title="{{ $meeting->person_address }}">
                                {{ $meeting->person_address ? Str::limit($meeting->person_address, 30) : '—' }}
                            </span>
                        </td>
                        <td style="text-align: center;">
                            @if($meeting->is_interview)
                                {!! icon('check') !!}
                            @endif
                        </td>
                        <td>
                            <span class="badge badge-{{ $meeting->meeting_status }}">
                                {{ $statuses[$meeting->meeting_status] ?? $meeting->meeting_status }}
                            </span>
                        </td>
                        <td>{{ $meeting->route?->route_name ?? '—' }}</td>
                        <td>{{ $meeting->maket?->maket_name ?? '—' }}</td>
                        <td>{{ $meeting->leaflets_issued ?: '—' }}</td>
                        <td>
                            @php
                                $sourceLabels = ['head_hunter' => 'Head Hunter', 'olx' => 'OLX', 'recommendation' => 'Рекомендация'];
                            @endphp
                            {{ $sourceLabels[$meeting->person_source] ?? '—' }}
                        </td>
                        <td style="max-width: 150px;">
                            <span style="display: -webkit-box; -webkit-line-clamp: 1; -webkit-box-orient: vertical; overflow: hidden;" 
                                title="{{ $meeting->meeting_comment }}">
                                {{ $meeting->meeting_comment ? Str::limit($meeting->meeting_comment, 20) : '—' }}
                            </span>
                        </td>
                        <td>
                            <div class="actions-cell">
                                <a href="{{ route('prom.meetings.edit', $meeting) }}" class="btn-icon" title="Редактировать">
                                    {!! icon('edit') !!}
                                </a>
                                <form method="POST" action="{{ route('prom.meetings.destroy', $meeting) }}" style="display: inline;"
                                      onsubmit="return confirm('Вы уверены, что хотите удалить эту встречу?')">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="15" style="text-align: center; color: #6b7280; padding: 2rem;">
